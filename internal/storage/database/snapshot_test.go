...
// encodeToBytes is a test helper that encodes the database to a byte slice.
func encodeToBytes(db *Database) ([]byte, error) {
	var buff bytes.Buffer
	bufWriter := bufio.NewWriter(&buff)
...
// decodeFromBytes is a test helper that decodes the database from a byte slice.
func decodeFromBytes(db *Database, data []byte) (numPeers, numTorrents int, err error) {
	tempTorrents := make(map[storage.Hash]*Torrent)
	numPeers, numTorrents, err = decode(db, bufio.NewReader(bytes.NewReader(data)), tempTorrents)
	if err != nil {
		return
	}
	db.mutex.Lock()
...
func TestBinaryCoder(t *testing.T) {
	...
	data, err := encodeToBytes(db)
	if err != nil {
		t.Fatal("encodeToBytes threw error: ", err)
	}
	...
	if _, _, err = decodeFromBytes(db, data); err != nil {
		t.Fatal("decodeFromBytes threw error: ", err)
	}
...
func TestBinaryCoderScaling(t *testing.T) {
	...
	for _, peers := range peerCounts {
		t.Run(fmt.Sprintf("%d_peers", peers), func(t *testing.T) {
			...
			data, err := encodeToBytes(originalDB)
			if err != nil {
				t.Fatal(err)
			}
			...
			numPeers, numTorrents, err := decodeFromBytes(newDB, data)
			if err != nil {
				t.Fatal(err)
			}
	...
	for peers := 1000; peers <= 5000000; peers *= 10 {
		b.Run(fmt.Sprintf("%d_peers", peers), func(b *testing.B) {
			...
			for i := 0; i < b.N; i++ {
				...
				_, _, err := decodeFromBytes(newDB, data)
				if err != nil {
					b.Fatal(err)
				}
			}
...
func TestIPv4AndIPv6(t *testing.T) {
	...
	numPeers, _, err := decodeFromBytes(newDB, data)
	if err != nil {
		t.Fatal(err)
	}
